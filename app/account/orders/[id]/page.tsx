...
const panel = "rounded-card border border-sand bg-paper p-6 shadow-sm";
const panelHeading = "font-display text-base font-bold text-espresso uppercase";
const badge =
    ...
    <div className="min-w-0">
      ...
      >
        ...
        <div className="mt-7 flex flex-wrap items-end gap-x-10 gap-y-5">
          <div>
            <p className="text-[0.62rem] font-semibold tracking-[0.24em] text-espresso-mute uppercase">
              Order total
            </p>
            <p className="font-display mt-2 text-[clamp(1.75rem,1.3rem+1.6vw,2.5rem)] leading-none font-extrabold tracking-[-0.04em] text-espresso">
      ...
      {order.cancelledAt && (
        <div className="mt-8 flex gap-3 rounded-card border border-red-200 bg-red-50 px-5 py-4">
          <Icon name="close" className="mt-0.5 size-4 shrink-0 text-red-700" />
      ...
      <div className="mt-12 flex flex-col gap-12">
        ...
        <section aria-labelledby="items-heading">
          <div className="flex items-baseline justify-between gap-6">
            ...
            {hasTracking && (
              <span className="hidden shrink-0 items-center gap-2 text-sm text-espresso-mute sm:flex">
          ...
          <div
            className={cn(
              ...
              returns30.items.length > 0 ? "bg-cream-deep" : "bg-sand/40",
            )}
          >
            <div className="min-w-0">
          ...
          <div className={panel}>
            ...
            <dl className="mt-5 space-y-2.5 text-sm">
              ...
              <div className="flex items-baseline justify-between gap-4 border-t border-sand pt-4">
                <dt className="font-display text-base font-bold text-espresso uppercase">
                  Total
                </dt>
                <dd className="font-display text-2xl font-bold tracking-[-0.03em] text-espresso">
                  {money(order.totalPrice)}
                </dd>
              </div>
            </dl>

            <div className="mt-5 space-y-1.5 border-t border-sand pt-4 text-xs text-espresso-mute">
              {currency && (
                <p>
                  Charged in <span className="text-espresso-soft">{currency}</span> —
          ...
          <div className={panel}>
            ...
            {order.shippingAddress ? (
              <div className="mt-5">
                ...
                <p className="mt-2.5 text-sm whitespace-pre-line text-espresso-soft">
                  {order.shippingAddress.formatted.join("\n")}
                </p>
              </div>
            ) : (
              <p className="mt-5 text-sm text-espresso-mute">
...
function Row({
  ...
  return (
    <div className="flex justify-between gap-4">
      ...
      <dd className={tone === "rose" ? "text-red-600" : "text-espresso"}>
        {value}
      </dd>
    </div>
  );
