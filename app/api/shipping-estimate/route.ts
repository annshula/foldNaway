 ...
 * freight API (lib/cj.ts) — a real per-address lookup. Destination country
 * comes from the same cookie/geo-detection the currency selector already
 * uses (lib/localization/country.ts), so the shopper only has to type a
 * pincode, not pick a country twice.
 */
