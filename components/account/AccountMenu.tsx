 ...
 * icon (no label) that routes to the Shopify Customer Account flow when
 * signed out.
 *
...
export function AccountMenu({
  ...
  dark?: boolean;
  /** Over the transparent mobile hero header: light text + translucent hover,
      reverting to the plain espresso look at `md:` — mirrors Logo.tsx and
      Nav.tsx's own `lightMobile` flag exactly. */
  lightMobile?: boolean;
  className?: string;
}) {
  ...
  const linkRow = (link: AccountLink, tone: "dark" | "light") => (
    <Link
      ...
      className={cn(
        ...
        tone === "dark"
          ? "rounded-full px-3 py-2.5 text-espresso hover:bg-cream-deep"
    ...
    >
      <Icon
        ...
        className={cn(
          "size-4 shrink-0",
          tone === "dark" ? "text-espresso" : "text-white/70",
        )}
      />
      {link.label}
    </Link>
  ...
  if (variant === "list") {
    const iconButton = cn(
      ...
      dark ? "text-white hover:bg-white/10" : "text-espresso hover:bg-espresso/5",
    );
  ...
  /* ── Desktop: icon button + dropdown, or a plain sign-in icon ────────── */
  if (signedIn === false) {
    return (
      <Link
        href="/account/login"
        aria-label="Sign in"
        className={cn(
          "hidden size-11 place-items-center rounded-full transition-colors duration-300 sm:grid",
          lightMobile
    ...
    <div ref={rootRef} className={cn("relative", className)}>
      ...
      {open && signedIn && (
        <div
          ...
          className="absolute top-full right-0 z-50 mt-2 w-56 overflow-hidden rounded-card border border-sand bg-paper p-1.5 shadow-(--shadow-lift)"
        >
          <div className="px-3 pt-2 pb-1.5">
            <p className="text-[0.62rem] font-semibold tracking-[0.24em] text-espresso uppercase">
              My account
            </p>
          </div>
          ...
          <div className="mt-1.5 border-t border-sand pt-1.5">
            <button
              ...
              role="menuitem"
              onClick={askSignOut}
              className="flex w-full items-center gap-2.5 rounded-full px-3 py-2.5 text-[0.72rem] font-semibold tracking-[0.14em] text-espresso-soft uppercase transition-colors duration-200 hover:bg-cream-deep hover:text-red-700"
            >
              <SignOutLabel />
            </button>
          </div>
