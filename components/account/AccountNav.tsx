 ...
 *
 * Desktop (lg+): a paper card of icon chips on the cream canvas, espresso
 * fill marking the current page, with sign-out under a hairline.
 *
...
export function AccountNav() {
  ...
  return (
    <nav aria-label="Account" className="w-full min-w-0">
      <div className="min-w-0 lg:rounded-card lg:border lg:border-sand lg:bg-paper lg:p-3 lg:shadow-sm">
        ...
        <ul className="scrollbar-none flex min-w-0 gap-7 overflow-x-auto border-b border-sand lg:flex-col lg:gap-1 lg:overflow-visible lg:border-b-0">
          {items.map((item) => {
              ...
              <li key={item.href} className="shrink-0 lg:shrink">
                <Link
                  ...
                  className={cn(
                    ...
                    active
                      ? "border-espresso text-espresso lg:bg-sage lg:text-on-accent"
                      : "border-transparent text-espresso-mute hover:text-espresso lg:text-espresso-soft lg:hover:bg-cream-deep lg:hover:text-espresso",
                  )}
                >
                  <Icon
                    name={item.icon}
        ...
        <div className="hidden lg:mt-2 lg:block lg:border-t lg:border-sand lg:pt-2">
          <button
            type="button"
            onClick={() => setSignOutOpen(true)}
            className="flex w-full items-center gap-2.5 rounded-full px-4 py-3 text-[0.72rem] font-semibold tracking-[0.14em] text-espresso-mute uppercase transition-colors duration-300 hover:bg-cream-deep hover:text-red-700"
          >
            <SignOutLabel />
          </button>
        </div>
