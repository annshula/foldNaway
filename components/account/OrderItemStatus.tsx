...
const accents: Record<StatusTone, { edge: string; chip: string }> = {
  neutral: {
    edge: "from-espresso/10 to-espresso/10",
    chip: "bg-espresso/5 text-espresso-soft",
  },
  rose: {
  ...
  ink: { edge: "from-sage to-sage", chip: "bg-sage text-on-accent" },
};
...
export function OrderItemStatus({
  ...
  const thumb = (size: string) =>
    item.image ? (
      ...
      <img
        ...
        className={cn("shrink-0 bg-cream-deep object-cover", size)}
      />
    ) : (
      <span
        className={cn(
          "grid shrink-0 place-items-center bg-cream-deep text-espresso-mute",
          size,
        )}
      >
  ...
  const face = (
    <>
      ...
      <Icon
        name="chevron-right"
        className="pointer-events-none absolute top-1/2 right-3 size-4 -translate-y-1/2 text-espresso-mute sm:hidden"
      />
      ...
      <span className="flex min-h-18 items-center gap-3.5 py-3.5 pr-2 pl-2 sm:pl-6">
        ...
        <span className="min-w-0 flex-1 pr-4 sm:pr-0">
          <span className="line-clamp-2 block text-sm leading-snug font-medium text-espresso sm:line-clamp-1">
            {item.title}
          </span>
          ...
          <span className="mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-1">
            {status}
            {marker?.at && (
              <span className="hidden text-[0.68rem] text-espresso-mute sm:inline">
                {shortDate(marker.at)}
              </span>
            )}
          </span>

          <span className="mt-1.5 block text-[0.7rem] text-espresso-mute sm:hidden">
            {meta}
          </span>
        </span>

        <span className="hidden shrink-0 text-right sm:block">
          <span className="block text-sm font-medium text-espresso">{price}</span>
          <span className="block text-[0.68rem] text-espresso-mute">
            ×{item.quantity}
          </span>
        </span>
        ...
        <Icon
          name="arrow-right"
          className="hidden size-4 shrink-0 text-espresso-mute transition-transform duration-500 ease-(--ease-out-expo) group-hover:translate-x-1 group-hover:text-espresso sm:block"
        />
      </span>
    </>
  );
  ...
  return (
    <ItemStatusPanel face={face} label={item.title}>
      <div className="flex min-h-0 flex-1 flex-col md:grid md:grid-cols-[minmax(0,17rem)_minmax(0,1fr)]">
        ...
        <div className="shrink-0 border-sand px-5 pt-4 pb-5 md:border-r md:bg-cream-deep md:px-7 md:py-8">
          <div className="flex items-start gap-4 md:flex-col md:gap-5">
            ...
            <div className="min-w-0">
              <p className="font-display font-semibold text-espresso md:text-lg">
                {item.title}
              </p>
              ...
              {item.sku && (
                <p className="mt-1 text-xs text-espresso-mute">{item.sku}</p>
              )}
            </div>
          </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 pt-6 pb-7 md:px-8 md:py-8">
          <h3 className="font-display text-lg font-bold text-espresso uppercase">
            Tracking
          </h3>
          <p className="mt-1.5 text-sm text-espresso-mute">
          ...
          {(description || reason || detail?.tracking?.number) && (
            <dl className="mt-7 space-y-4 border-t border-sand pt-5 text-sm">
              {description && (
                <div>
                  <dd className="leading-relaxed text-espresso-soft">
                    {description}
                  </dd>
                </div>
              )}
              {reason && (
                <div>
                  <dt className="text-[0.62rem] font-semibold tracking-[0.24em] text-espresso-mute uppercase">
                    Reason
                  </dt>
                  <dd className="mt-1.5 text-espresso">
                  ...
                  <dd className="mt-1.5">
                    {detail.tracking.url ? (
                      <Link
                        ...
                        className="text-espresso underline underline-offset-4 hover:text-espresso-soft"
                      >
                        {detail.tracking.number}
                      </Link>
                    ) : (
                      <span className="text-espresso">{detail.tracking.number}</span>
                    )}
                  </dd>
                </div>
              )}
        ...
        return (
          <li key={node.id} className="relative flex gap-4 pb-6 last:pb-0">
            ...
            {!isLast && (
              <span
                aria-hidden="true"
                className="absolute top-5 bottom-0 left-1.75 w-0.5 overflow-hidden rounded-full bg-sand"
              >
...
function Dot({ state }: { state: JourneyState }) {
  ...
  if (state === "pending") {
    return (
      <span className="grid size-4 place-items-center rounded-full border border-dashed border-espresso-mute/50 bg-paper">
        <span className="size-1 rounded-full bg-sand" />
      </span>
    );
  }
  return (
    <span className="grid size-4 place-items-center rounded-full bg-emerald-500 text-espresso ring-[3px] ring-emerald-500/25">
