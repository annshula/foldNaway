...
const labelClass =
  "text-[0.7rem] font-semibold tracking-[0.14em] text-espresso-soft uppercase";
...
export function ReturnRequestForm({
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem] lg:items-start">
      <ul className="flex flex-col gap-4">
        {items.map((item) => {
          ...
          return (
            <li
              key={item.id}
              className="rounded-card border border-sand bg-paper p-5 shadow-sm sm:p-6"
            >
              <div className="grid grid-cols-[auto_minmax(0,1fr)] items-start gap-x-4 gap-y-5 sm:grid-cols-[auto_minmax(0,1fr)_auto]">
                {item.image ? (
                  ...
                  <img
                    ...
                    className="size-16 shrink-0 rounded-xl bg-cream-deep object-cover"
                  />
                ) : (
                  <span className="grid size-16 shrink-0 place-items-center rounded-xl bg-cream-deep text-espresso-mute">
                ...
                <div className="col-start-2 flex items-center justify-end gap-3 sm:col-start-3">
                  <span className="text-[0.7rem] font-semibold tracking-[0.14em] text-espresso-soft uppercase sm:hidden">
                    Returning
                  </span>
                  <QuantityStepper
                    value={quantity}
                  ...
                  <label className="flex flex-col gap-2">
                    ...
                    <div className="relative">
                      <select
                        ...
                        className="w-full cursor-pointer appearance-none rounded-full border border-sand bg-cream-deep px-4 py-3 pr-11 text-sm text-espresso transition-colors duration-300 hover:border-espresso/30 focus:border-espresso focus:outline-none"
                      >
                    ...
                    <>
                      <label className="flex flex-col gap-2">
                        ...
                        <textarea
                          ...
                          className="w-full resize-none rounded-2xl border border-sand bg-cream-deep px-4 py-3 text-sm text-espresso transition-colors duration-300 hover:border-espresso/30 focus:border-espresso focus:outline-none"
                        />
                      </label>
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <input
                          ...
                          className="cursor-pointer text-sm text-espresso-soft file:mr-3 file:cursor-pointer file:rounded-full file:border-0 file:bg-espresso file:px-4 file:py-2 file:text-xs file:font-semibold file:tracking-wide file:text-cream file:uppercase"
                        />
                        {evidence[item.id]?.uploading && (
                          <p className="text-xs text-espresso-mute">Uploading…</p>
                        )}
                        {!evidence[item.id]?.uploading &&
      ...
      <aside className="rounded-card border border-sand bg-paper p-6 shadow-sm lg:sticky lg:top-24">
        <h2 className="font-display text-lg font-bold text-espresso uppercase">
          Summary
        </h2>
        ...
        <div className="mt-5 flex items-baseline justify-between gap-4">
          ...
          <span className="font-display text-3xl font-bold tracking-[-0.03em] text-espresso">
        ...
        <p className="mt-4 text-xs text-espresso-mute">
...
function QuantityStepper({
  ...
  const stepClass =
    "grid size-10 cursor-pointer place-items-center text-espresso transition-colors duration-300 hover:text-espresso disabled:cursor-not-allowed disabled:opacity-35";

  return (
    <div className="flex shrink-0 flex-col items-end gap-1.5">
      <div className="flex items-center rounded-full border border-sand bg-cream-deep">
        <button
          type="button"
        ...
        <span
          ...
          className="w-8 text-center font-medium text-espresso"
        >
          {value}
        </span>
        <button
      ...
      <span className="text-xs text-espresso-mute">of {max}</span>
    </div>
  );
}
