    ...
    >
      ...
      <div className="absolute inset-0 grid place-items-center p-5">
        <div
          ...
          tabIndex={-1}
          className={cn(
            "relative w-full max-w-sm overflow-hidden rounded-card bg-paper shadow-(--shadow-lift) transition-all duration-300 ease-(--ease-out-expo)",
            open
              ? "translate-y-0 scale-100 opacity-100"
              : "translate-y-4 scale-95 opacity-0",
          )}
        >
          {/* Espresso header band with the logout glyph on the seam. */}
          <div className="relative h-20 bg-espresso">
            <span className="absolute bottom-0 left-1/2 z-10 grid size-16 -translate-x-1/2 translate-y-1/2 place-items-center rounded-full border-4 border-paper bg-espresso text-cream shadow-(--shadow-lift)">
          ...
          <div className="px-6 pt-12 pb-7 text-center">
            <h2
              id="signout-title"
              className="font-display text-2xl font-extrabold tracking-[-0.03em] text-espresso"
            >
              Leaving so soon?
            </h2>
            <p
              id="signout-body"
              className="mx-auto mt-2 max-w-xs text-sm leading-relaxed text-espresso-soft"
            >
            ...
            <div className="mt-7 flex flex-col-reverse gap-3">
              <button
                ...
                onClick={confirm}
                disabled={leaving}
                className="text-[0.72rem] font-semibold tracking-[0.16em] text-espresso-mute uppercase transition-colors duration-300 hover:text-red-700 disabled:opacity-50"
              >
