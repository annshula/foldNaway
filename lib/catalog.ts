...
export const foldnawayProductIds = new Set(syncedProducts.map((p) => p.id));

/** Every variant of every one of our products; anything else on the shared store belongs to another brand. */
const foldnawayVariantIds = new Set(
  syncedProducts.flatMap((p) => p.variants.map((v) => v.id)),
);

/**
 * Whether an order line item belongs to FoldNAway. Matches the product id
 ...
 */
export function belongsToFoldNAway(input: {
  variantId?: string | null;
  productId?: string | null;
}): boolean {
  if (input.productId && foldnawayProductIds.has(input.productId)) return true;
  return Boolean(input.variantId && foldnawayVariantIds.has(input.variantId));
}
